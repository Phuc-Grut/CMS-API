...
public class UpgradeController : ControllerBase
{
    ...
    public UpgradeController(IMediatorHandler mediator, IContextUser context, ILogger<ContentController> logger, CodeSyntaxConfig codeSyntax)
    {
        _mediator = mediator;
        _context = context;
        _logger = logger;
    ...
    {
        var query = new ContentPagingFilterQuery(request?.Keyword, "UPGRADE", request?.Filter, request?.Order, request.PageNumber, request.PageSize);
    ...
    public async Task<IActionResult> GetByLevel(string? parent, int? level, int? levelCount)
    {
        ...
        //var result = await _mediator.Send(new CategoryQueryByLevel(1, "UPGRADE", parent, level, levelCount));
        return Ok(result);
    ...
    [HttpPost("add")]
    public async Task<IActionResult> Add([FromBody] AddUpgradeRequest request)
    {
    ...
    [HttpPut("edit")]
    public async Task<IActionResult> Put([FromBody] EditUpgradeRequest request)
    {

        var cmd = new UpgradeEditCommand()
        {
            Id = request.Id,
            Code = request.Code,
            Name = request.Name,
    ...
    [HttpPost("duplicate")]
    public async Task<IActionResult> Duplicate([FromBody] DuplicateUpgradeRequest request)
    {
        ...
        var contentDuplicateCommand = new UpgradeDuplicateCommand(
            ContentId,
            code,
            request.Name,
            1,
