...
public class GuideController : ControllerBase
{
    ...
    public GuideController(IMediatorHandler mediator, IContextUser context, ILogger<ContentController> logger, CodeSyntaxConfig codeSyntax)
    {
        _mediator = mediator;
        _context = context;
        _logger = logger;
    ...
    {
        var query = new ContentPagingFilterQuery(request?.Keyword, "GUIDE", request?.Filter, request?.Order, request.PageNumber, request.PageSize);
        var result = await _mediator.Send(query);
        return Ok(result);
    }
    [HttpGet("get-cate")]
    public async Task<IActionResult> GetCategoryChannel()
    {

        // var result = await _mediator.Send(new CategoryQueryByLevel(1, "GUIDE", parent, level, levelCount));
    ...
    [HttpPost("add")]
    public async Task<IActionResult> Add([FromBody] AddGuideRequest request)
    {
    ...
    [HttpPut("edit")]
    public async Task<IActionResult> Put([FromBody] EditGuideRequest request)
    {

        var cmd = new GuideEditCommand()
        {
            Id = request.Id,
            Code = request.Code,
            Name = request.Name,
    ...
    [HttpPost("duplicate")]
    public async Task<IActionResult> Duplicate([FromBody] DuplicateGuideRequest request)
    {
        ...
        var contentDuplicateCommand = new GuideDuplicateCommand(
            ContentId,
            code,
            request.Name,
            1,
