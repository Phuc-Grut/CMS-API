...
public class NoticeController : ControllerBase
{
    ...
    public NoticeController(IMediatorHandler mediator, IContextUser context, ILogger<ContentController> logger, CodeSyntaxConfig codeSyntax)
    {
        _mediator = mediator;
        _context = context;
        _logger = logger;
    ...
    {
        var query = new ContentPagingFilterQuery(request?.Keyword, "NOTICE", request?.Filter, request?.Order, request.PageNumber, request.PageSize);
    ...
    public async Task<IActionResult> GetByLevel(string? parent, int? level, int? levelCount)
    {
        ...
        //var result = await _mediator.Send(new CategoryQueryByLevel(1, "NOTICE", parent, level, levelCount));
    ...
    [HttpPost("add")]
    public async Task<IActionResult> Add([FromBody] AddNoticeRequest request)
    {
    ...
    [HttpPut("edit")]
    public async Task<IActionResult> Put([FromBody] EditNoticeRequest request)
    {

        var cmd = new NoticeEditCommand()
        {
            Id = request.Id,
            Code = request.Code,
            Name = request.Name,
    ...
    [HttpPost("duplicate")]
    public async Task<IActionResult> Duplicate([FromBody] DuplicateNoticeRequest request)
    {
        ...
        var contentDuplicateCommand = new NoticeDuplicateCommand(
            ContentId,
            code,
            request.Name,
            1,
