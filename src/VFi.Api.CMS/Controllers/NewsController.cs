...
public class NewsController : ControllerBase
{
    ...
    public NewsController(IMediatorHandler mediator, IContextUser context, ILogger<ContentController> logger, CodeSyntaxConfig codeSyntax)
    {
        _mediator = mediator;
        _context = context;
        _logger = logger;
    ...
    {
        var query = new ContentPagingFilterQuery(request?.Keyword, "NEWS", request?.Filter, request?.Order, request.PageNumber, request.PageSize);
    ...
    public async Task<IActionResult> GetByLevel(string? parent, int? level, int? levelCount)
    {
        ...
        //var result = await _mediator.Send(new CategoryQueryByLevel(1, "NEWS", parent, level, levelCount));
    ...
    [HttpPost("add")]
    public async Task<IActionResult> Add([FromBody] AddNewsRequest request)
    {
    ...
    [HttpPut("edit")]
    public async Task<IActionResult> Put([FromBody] EditNewsRequest request)
    {

        var cmd = new NewsEditCommand()
        {
            Id = request.Id,
            Code = request.Code,
            Name = request.Name,
    ...
    [HttpPost("duplicate")]
    public async Task<IActionResult> Duplicate([FromBody] DuplicateNewsRequest request)
    {
        ...
        var contentDuplicateCommand = new NewsDuplicateCommand(
            ContentId,
            code,
            request.Name,
            1,
