...
internal class NewsCommandHandler : CommandHandler, IRequestHandler<NewsAddCommand, ValidationResult>,
                                                        IRequestHandler<NewsDeleteCommand, ValidationResult>,
                                                        IRequestHandler<NewsEditCommand, ValidationResult>,
                                                        IRequestHandler<NewsDuplicateCommand, ValidationResult>
{
    ...
    public async Task<ValidationResult> Handle(NewsAddCommand request, CancellationToken cancellationToken)
    {
        if (!request.IsValid(_contentRepository))
            return request.ValidationResult;
        var contentType = await _contentTypeRepository.GetByCode("News");
        ...
        var channel = await _groupCategoryRepository.GetByCode("News");
    ...
    public async Task<ValidationResult> Handle(NewsDeleteCommand request, CancellationToken cancellationToken)
    {
        if (!request.IsValid(_contentRepository))
            return request.ValidationResult;
    ...
    public async Task<ValidationResult> Handle(NewsEditCommand request, CancellationToken cancellationToken)
    {
        if (!request.IsValid(_contentRepository))
            return request.ValidationResult;
    ...
    public async Task<ValidationResult> Handle(NewsDuplicateCommand request, CancellationToken cancellationToken)
    {
        if (!request.IsValid(_contentRepository))
            return request.ValidationResult;
