...
internal class NoticeCommandHandler : CommandHandler, IRequestHandler<NoticeAddCommand, ValidationResult>,
                                                        IRequestHandler<NoticeDeleteCommand, ValidationResult>,
                                                        IRequestHandler<NoticeEditCommand, ValidationResult>,
                                                        IRequestHandler<NoticeDuplicateCommand, ValidationResult>
{
    ...
    public async Task<ValidationResult> Handle(NoticeAddCommand request, CancellationToken cancellationToken)
    {
        if (!request.IsValid(_contentRepository))
            return request.ValidationResult;
        var contentType = await _contentTypeRepository.GetByCode("Notice");
        ...
        var channel = await _groupCategoryRepository.GetByCode("Notice");
    ...
    public async Task<ValidationResult> Handle(NoticeDeleteCommand request, CancellationToken cancellationToken)
    {
        if (!request.IsValid(_contentRepository))
            return request.ValidationResult;
    ...
    public async Task<ValidationResult> Handle(NoticeEditCommand request, CancellationToken cancellationToken)
    {
        if (!request.IsValid(_contentRepository))
            return request.ValidationResult;
    ...
    public async Task<ValidationResult> Handle(NoticeDuplicateCommand request, CancellationToken cancellationToken)
    {
        if (!request.IsValid(_contentRepository))
            return request.ValidationResult;
