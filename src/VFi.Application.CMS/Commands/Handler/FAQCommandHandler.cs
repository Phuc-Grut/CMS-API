...
internal class FaqCommandHandler : CommandHandler, IRequestHandler<FaqAddCommand, ValidationResult>,
                                                        IRequestHandler<FaqDeleteCommand, ValidationResult>,
                                                        IRequestHandler<FaqEditCommand, ValidationResult>,
                                                        IRequestHandler<FaqDuplicateCommand, ValidationResult>
{
    ...
    public async Task<ValidationResult> Handle(FaqAddCommand request, CancellationToken cancellationToken)
    {
        if (!request.IsValid(_contentRepository))
            return request.ValidationResult;
        var contentType = await _contentTypeRepository.GetByCode("FAQ");
        ...
        var channel = await _groupCategoryRepository.GetByCode("FAQ");
    ...
    public async Task<ValidationResult> Handle(FaqDeleteCommand request, CancellationToken cancellationToken)
    {
        if (!request.IsValid(_contentRepository))
            return request.ValidationResult;
    ...
    public async Task<ValidationResult> Handle(FaqEditCommand request, CancellationToken cancellationToken)
    {
        if (!request.IsValid(_contentRepository))
            return request.ValidationResult;
    ...
    public async Task<ValidationResult> Handle(FaqDuplicateCommand request, CancellationToken cancellationToken)
    {
        if (!request.IsValid(_contentRepository))
            return request.ValidationResult;
